...
def file_to_image(file_path, output_path):
    ...
    img_size = int(np.ceil(np.sqrt(len(byte_array))))

    # Pad byte array if needed
    padded_array = np.pad(byte_array, (0, img_size**2 - len(byte_array)), mode='constant', constant_values=0)

    # Reshape into 2D image
    image = padded_array.reshape((img_size, img_size))
    ...
    plt.title("Binary File Visualization")
    plt.axis("off")
    plt.show()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(file_path) or not os.path.isfile(file_path):
        ...
        file_to_image(file_path, output_path)
