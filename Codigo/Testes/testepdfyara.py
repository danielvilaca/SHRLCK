...
def yara_scan(file_path, yara_rules_path):
    try:
        ...
        if matches:
            print(f"  YARA rules matched: {', '.join([match.rule for match in matches])}")
        else:
...
def analyze_pdf(file_path, yara_rules_path):
    ...
    # YARA
    yara_scan(file_path, yara_rules_path)

    print("PDF Analysis Complete.\n")

if __name__ == "__main__":
    file_path = input("Enter the path to the PDF file: ").strip()
    yara_rules_path = input("Enter the path to the YARA rules file: ").strip()
    analyze_pdf(file_path, yara_rules_path)
